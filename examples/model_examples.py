...
#    print
    ...
    checkM(forall(x, even(x) >> Not(even(suc(x)))))
    ...
    checkM(forall([x, y], And(prime(x), prime(y), x < y) >> Not(even(y))))
    ...
    checkM(exists(x, And(x < ii(10), forall(y, And(x < y, implies(prime(y), Not(even(y))))))))

    def precond(n):
        return And((ii(2) < n), even(n))
