...
#
# Func_term(f,[t1,...,tn]) represents a function f appliied to n arguments t1...tn
#  f(t1,t2,...,tn)
#
...
class Const(Term):
    ...
    def __div__(self,other):
        if isinstance(other,(int,float,Fraction)):
            ...
            coeff = (1/Fraction(other) if isinstance(other,float) else Fraction(1,other))
...
class Add_pair:
    ...
    def __div__(self, factor):
        num = (Fraction(self.coeff) if isinstance(self.coeff,float) else self.coeff)
...
class Add_term(Term):
    ...
    def __div__(self, factor):
        if isinstance(factor,(int,float,Fraction)):
            return Add_term([s / (Fraction(factor) if isinstance(factor,float) else factor) for s in self.addpairs])
...
class Mul_pair:
    ...
    def __pow__(self, n):
        if isinstance(n,int) or (isinstance(n,Fraction) and n.denominator%2==1):
...
class Func_term(Term):
    ...
    def __add__(self,other):
        if isinstance(other,Add_term):
            return other + self
        if isinstance(other,Func_term) and other.name == self.name and other.args == self.args:
    ...
    def lt_rel(self,other,H):
        ...
        if wsignj!=0: 
            ...
            if (i,j) in H.term_comparisons.keys():
                ...
                for c in comps:
                    if ((wsignj==1 and ((c.comp == LT and c.coeff <= 1) or (c.comp == LE and c.coeff < 1))) or
        ...
        if (i,j) in H.term_comparisons.keys():
            comps = H.term_comparisons[i,j]
            if (any((c.comp==LT and c.coeff<=1) or (c.comp==LE and c.coeff<1) for c in comps) and
                any(((c.comp == LT and (c.coeff < 0 or c.coeff >= 1))or (c.comp == LE and (c.coeff < 0 or c.coeff > 1))) for c in comps)):
                return True
        return False
    ...
    def gt_rel(self,other,H):
        ...
        if wsignj!=0: 
            ...
            if (i,j) in H.term_comparisons.keys():
                comps = H.term_comparisons[i,j]
                for c in comps:
                    if ((wsignj==1 and ((c.comp == GT and c.coeff >= 1) or (c.comp == GE and c.coeff > 1))) or
                        (wsignj==-1 and ((c.comp == GT and c.coeff <= 1) or (c.comp == GE and c.coeff < 1 )))):
                        return True
            return False
        ...
        if (i,j) in H.term_comparisons.keys():
            comps = H.term_comparisons[i,j]
            if (any((c.comp == GT and c.coeff >= 1) or (c.comp == GE and c.coeff > 1) for c in comps) and
                any((c.comp == GT and c.coeff <= 1) or (c.comp == GE and c.coeff < 1) for c in comps)):
                return True
        return False
